      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="py-3 sm:py-4 space-y-3 sm:space-y-4">
          <div className="bg-muted p-3 sm:p-4 rounded-lg space-y-2">
            <div className="flex justify-between items-center">
              <span className="text-xs sm:text-sm text-muted-foreground">Τρέχον Πακέτο:</span>
              <Badge variant="outline">{packageType}</Badge>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs sm:text-sm text-muted-foreground">Υπόλοιπες Συνεδρίες:</span>
        ...
        <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-2">
          ...
          <Button onClick={() => { navigate('/store'); onOpenChange(false); }} className="w-full sm:w-auto">
            Δείτε Πακέτα
          </Button>
        </DialogFooter>
      </DialogContent>
...
export const ChatMessageNotification: React.FC<NotificationComponentProps> = ({
  ...
  const senderName = data?.sender_name || 'Γραμματεία SWEAT93';
...
export const NewEventNotification: React.FC<NotificationComponentProps> = ({
  ...
  const eventLocation = data?.event_location || 'Sweat93 Gym';
